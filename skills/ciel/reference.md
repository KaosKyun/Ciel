...
| Agent dispatch without `subagent_type` | `Agent(description="...", prompt="...")` without `subagent_type="ciel-*"` spawns a generic Claude agent — no Haiku model, no call caps, no waterfall early-exit → 20K-160K tokens burned vs ~5K for a constrained Ciel agent. Observed in production sessions. | Implement `pre-agent-gate.sh` PreToolUse hook on `Agent` tool: read `tool_input.subagent_type`; if absent or not prefixed with `ciel-`, deny with "Use subagent_type=ciel-researcher\|ciel-explorer\|ciel-critic\|ciel-improver". Pending implementation. |
| Hooks directory absent | `~/.claude/plugins/ciel/hooks/` missing entirely (new Claude Code install, machine migration, or accidental deletion) → every Write/Stop hook fires "No such file or directory" as non-blocking error; FAIRE gates and meta-critiquer silently skip. | On `ciel-init` and after any Claude Code reinstall: verify `ls ~/.claude/plugins/ciel/hooks/` returns ≥9 scripts. If absent: `mkdir -p ~/.claude/plugins/ciel/hooks && cp <repo>/Ciel/hooks/*.sh ~/.claude/plugins/ciel/hooks/`. Add to `ciel-init` checklist. |
