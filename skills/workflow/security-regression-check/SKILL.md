...
description: After FAIRE on Critical tasks, greps the diff for NEW inputs from request params, removed authentication blocks, or new external calls — reads the diff with attacker eyes. Any new surface found is treated as a Critical finding in RELIRE. Runs between faire-gatekeeper and relire-critic on Critical tasks only.
