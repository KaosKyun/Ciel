...
description: Runs a 3-pass STRIDE threat model on a Critical coding change with grep evidence per category — PASSE 1 risk-rank, PASSE 2 STRIDE 6 categories (Spoofing/Tampering/Repudiation/Info Disclosure/DoS/Elevation), PASSE 3 killer checklist. Mandatory for Critical tasks (auth, DB schema, payment, security). Produces evidence-backed findings, not theater.
