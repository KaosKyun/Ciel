...
- [ ] HTTP methods carry correct semantics (GET idempotent, POST non-idempotent)?
- [ ] Versioning strategy consistent (URL path OR header, not mixed)?
- [ ] Pagination present on all list endpoints?
- [ ] Error shape uniform across all endpoints?
- [ ] Transport auth parity (REST and WS use same auth)?
